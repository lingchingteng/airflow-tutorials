...
def process_metadata(mode, **context):
    ...
    if mode == "read":
        ...
        _, all_comic_info = context["task_instance"].xcom_pull(
            task_ids="check_comic_info"
        )

        # update to latest chapter
        for comic_id, comic_info in dict(all_comic_info).items():
            all_comic_info[comic_id]["previous_chapter_num"] = comic_info[
                "latest_chapter_num"
            ]
...
def check_comic_info(**context):
    ...
    for comic_id, comic_info in dict(all_comic_info).items():
        ...
        latest_chapter_num = [
            int(s) for s in links[-1].text.split() if s.isdigit()
        ][0]
        previous_chapter_num = comic_info["previous_chapter_num"]

        all_comic_info[comic_id]["latest_chapter_num"] = latest_chapter_num
        all_comic_info[comic_id]["new_chapter_available"] = (
            latest_chapter_num > previous_chapter_num
        )
        if all_comic_info[comic_id]["new_chapter_available"]:
            anything_new = True
            print(
                "There are new chapter for {}(latest: {})".format(
                    comic_name, latest_chapter_num
                )
            )
...
def decide_what_to_do(**context):
    anything_new, all_comic_info = context["task_instance"].xcom_pull(
        task_ids="check_comic_info"
    )
...
with DAG(
    "comic_app_v3", default_args=default_args, schedule_interval="@daily"
) as dag:
